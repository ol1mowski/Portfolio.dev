...
test.describe('E-book Download Flow', () => {
  test('should complete full e-book download process', async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: /Akceptuję Politykę Prywatności/i }).check();
    ...
    await page.waitForURL('/materialy');

    await expect(page).toHaveURL('/materialy');

    // User should be redirected to materials page after successful submission
  ...
  test('should validate form fields', async ({ page }) => {
    ...
    await expect(page.getByText('*Musisz zaakceptować politykę prywatności')).toBeVisible();
